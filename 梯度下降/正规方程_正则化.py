...
def normalEqn(data, lamb):
    x0 = np.mat([1]*data.shape[0]).T
    x1 = np.mat(data.iloc[:, :-1])
    x = np.hstack((x0, x1))
    y = np.mat(data.iloc[:, -1]).T
   # theta = (np.linalg.pinv(x.T.dot(x)).dot(x.T)).dot(y)
    theta = (np.linalg.pinv(x.T*x + lamb * np.diag([0] + [1] * (data.shape[1]-1)))*x.T)*y
    cost_value = computeCost(x, y, theta.T, lamb)
    return theta, cost_value

# 损失函数
def computeCost(x, y, theta, lamb):
    inner = np.power(x * theta.T - y, 2) + lamb * np.sum(np.power(theta[0, 1:], 2))
    return np.sum(inner) / (2*len(x))

best_theta, cost_value = normalEqn(data, lamb=10000000)
print(best_theta)
